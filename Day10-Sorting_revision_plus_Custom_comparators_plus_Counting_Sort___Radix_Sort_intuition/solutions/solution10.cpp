/*
 * Solution 10: Interval Sort (Sorting revision plus Custom comparators plus Counting Sort Radix Sort intuition)
 */
#include <iostream>
#include <vector>
#include <algorithm>
...
class Solution {
public:
// --- Function Explanation: interval_sort ---
// Purpose    : Reorder data according to problem rule in `interval_sort`.
...
// 4) Return sorted/rearranged sequence or computed metric.
    int interval_sort(vector<int>& nums) {
        int ans = 0;
...
int main() {
    ...
    Solution s; cout << s.interval_sort(nums) << "\n"; return 0;
}
