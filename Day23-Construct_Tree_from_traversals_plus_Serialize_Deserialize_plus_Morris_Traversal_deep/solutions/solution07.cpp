/*
 * Solution 7: Kth Smallest BST (Construct Tree from traversals plus Serialize Deserialize plus Morris Traversal deep)
 */
#include <iostream>
#include <queue>
using namespace std;
...
// 4) Return final computed result.
    int kth_smallest_bst(TreeNode* root) {
        if (!root) return 0;
        return 1 + kth_smallest_bst(root->left) + kth_smallest_bst(root->right);
    }
};
...
int main() {
    ...
    Solution s; cout << s.kth_smallest_bst(r) + 7 << "\n"; return 0;
}
